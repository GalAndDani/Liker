    ...
    setTimeout(function () {
        if (popUpWindow && popUpWindow.closed) {
            $.ajax({
                ...
                type: 'POST',
                url: '/liker/public/get-points/facebook/check/photo-like',
                data: {
                ...
                success: function (response) {
                    if(response == 'liked') {
                        ...
                        $('#status').html('Successfully liked, You got ' + points + ' Points');
                    } else {
                        $('#status').removeClass('alert-success');
                        $('#status').addClass('alert-danger');
                        $('#status').html('Not liked. You got 0 Points');
                    }
                    getFaceBookPost();
                }
            });
...
function getFaceBookPost() {
    ...
    $.ajax({
        type: 'GET',
        url: '/liker/public/get-points/facebook/pages/photo-likes',
        success: function (response) {
            if(response == 'no-result') {
                ...
                $('#posts').append('<div class="ui card" id="page" style="margin: 0 auto;">' +
                    ...
                    '<span id="like" data-url="' + response[0]['url'] + '"><i class="fa fa-heart" aria-hidden="true"></i> Like</span>' +
                    '</span>' +
                    '</div>' +
